...
func _build_ui() -> void:
	...
	var top_h: float = clampf(size.y * 0.112, _d(94, scale), _d(126, scale))
...
func _build_top_band(rect: Rect2, scale: float) -> void:
	...
	_label("当前挑战", Rect2(rect.position + Vector2(pad, _d(13, scale)), Vector2(left_w - pad * 2.0, _d(16, scale))), 12, HudReferenceTheme.GOLD, scale, false)
	_label("消费信心下降", Rect2(rect.position + Vector2(pad, _d(32, scale)), Vector2(left_w - pad * 2.0, _d(32, scale))), 28, HudReferenceTheme.TITLE, scale, true)
	_label("核心变量：C↓，总需求下降，IS 曲线左移。", Rect2(rect.position + Vector2(pad, _d(67, scale)), Vector2(left_w - pad * 2.0, _d(25, scale))), 15, HudReferenceTheme.BODY, scale, false)
	...
	_label("回合 1 / 1", Rect2(Vector2(center_x, rect.position.y + _d(17, scale)), Vector2(center_w, _d(28, scale))), 22, HudReferenceTheme.TITLE, scale, true, HORIZONTAL_ALIGNMENT_CENTER)
	...
	_label("公元1000年 第一季度", Rect2(Vector2(rx, rect.position.y + _d(16, scale)), Vector2(right_w - _d(38, scale), _d(22, scale))), 14, HudReferenceTheme.MUTED, scale, false, HORIZONTAL_ALIGNMENT_RIGHT)
...
func _build_left_drawer(rect: Rect2, scale: float) -> void:
	...
	_label("政策卡", Rect2(rect.position + Vector2(_d(18, scale), _d(16, scale)), Vector2(rect.size.x - _d(72, scale), _d(28, scale))), 22, HudReferenceTheme.TITLE, scale, true)
...
func _build_policy_card(rect: Rect2, data: Dictionary, selected: bool, scale: float) -> void:
	...
	_label(String(data["title"]), Rect2(rect.position + Vector2(_d(72, scale), _d(18, scale)), Vector2(rect.size.x - _d(94, scale), _d(25, scale))), 17, HudReferenceTheme.TITLE, scale, true)
	_label(String(data["desc"]), Rect2(rect.position + Vector2(_d(72, scale), _d(48, scale)), Vector2(rect.size.x - _d(92, scale), _d(42, scale))), 13, HudReferenceTheme.BODY, scale, false)
...
func _build_right_drawer(rect: Rect2, scale: float) -> void:
	...
	_label("宏观状态", Rect2(rect.position + Vector2(_d(20, scale), _d(18, scale)), Vector2(rect.size.x - _d(40, scale), _d(28, scale))), 22, HudReferenceTheme.TITLE, scale, true)
	_label("当前问题：需求不足", Rect2(rect.position + Vector2(_d(20, scale), _d(54, scale)), Vector2(rect.size.x - _d(40, scale), _d(22, scale))), 13, HudReferenceTheme.MUTED, scale, false)
	_label("核心指标", Rect2(rect.position + Vector2(_d(20, scale), _d(90, scale)), Vector2(rect.size.x - _d(40, scale), _d(21, scale))), 15, HudReferenceTheme.GOLD, scale, true)
	...
	var metrics: Array[Dictionary] = [
		...
		{"name": "债务", "symbol": "Debt", "value": "60.0%", "status": "适中", "n": 0.57, "r": 0.50},
	]
	...
	_label("行动提示", Rect2(rect.position + Vector2(_d(20, scale), rect.size.y - _d(128, scale)), Vector2(rect.size.x - _d(40, scale), _d(20, scale))), 14, HudReferenceTheme.GOLD, scale, true)
	_label("选择一张政策卡，并确认执行。", Rect2(rect.position + Vector2(_d(20, scale), rect.size.y - _d(103, scale)), Vector2(rect.size.x - _d(40, scale), _d(30, scale))), 13, HudReferenceTheme.BODY, scale, false)
	...
	_label("确认政策", Rect2(button_rect.position, button_rect.size), 16, HudReferenceTheme.TITLE, scale, true, HORIZONTAL_ALIGNMENT_CENTER, VERTICAL_ALIGNMENT_CENTER)
...
func _build_bottom_analysis(rect: Rect2, scale: float) -> void:
	...
	_label("IS-LM 分析", Rect2(islm_rect.position + Vector2(_d(16, scale), _d(11, scale)), Vector2(islm_rect.size.x - _d(32, scale), _d(24, scale))), 18, HudReferenceTheme.TITLE, scale, true)
	_label("消费信心下降 → C 减少 → 总需求下降 → IS 曲线左移", Rect2(islm_rect.position + Vector2(_d(16, scale), _d(38, scale)), Vector2(islm_rect.size.x - _d(32, scale), _d(24, scale))), 13, HudReferenceTheme.BODY, scale, false)
	var chart: Control = HudReferenceChart.new()
	chart.mouse_filter = Control.MOUSE_FILTER_IGNORE
	chart.position = islm_rect.position + Vector2(_d(14, scale), _d(66, scale))
	chart.size = islm_rect.size - Vector2(_d(28, scale), _d(78, scale))
	chart.setup_islm()
	add_child(chart)

	_label("总需求构成", Rect2(donut_rect.position + Vector2(_d(16, scale), _d(11, scale)), Vector2(donut_rect.size.x - _d(32, scale), _d(24, scale))), 18, HudReferenceTheme.TITLE, scale, true)
...
func _label(text: String, rect: Rect2, font_size: int, color: Color, scale: float, strong: bool = false, align: HorizontalAlignment = HORIZONTAL_ALIGNMENT_LEFT, valign: VerticalAlignment = VERTICAL_ALIGNMENT_TOP) -> Label:
	...
	label.add_theme_font_size_override("font_size", HudReferenceTheme.font(font_size, scale))
	label.add_theme_color_override("font_color", color)
	if strong:
		HudReferenceTheme.add_text_shadow(label, 0.58)
	add_child(label)
	return label
...
func _chip(text: String, rect: Rect2, accent: Color, scale: float, icon_type: String = "none") -> void:
	...
	_label(text, text_rect, 12, accent if icon_type == "none" else HudReferenceTheme.TITLE, scale, false, HORIZONTAL_ALIGNMENT_CENTER, VERTICAL_ALIGNMENT_CENTER)


func _resource_chip(icon_type: String, text: String, rect: Rect2, accent: Color, scale: float) -> void:
	_chip(text, rect, accent, scale, icon_type)
...
func _icon_button(icon_type: String, text: String, rect: Rect2, accent: Color, scale: float) -> void:
	...
	_label(text, Rect2(rect.position + Vector2(_d(28, scale), 0), Vector2(rect.size.x - _d(30, scale), rect.size.y)), 12, HudReferenceTheme.BODY, scale, false, HORIZONTAL_ALIGNMENT_CENTER, VERTICAL_ALIGNMENT_CENTER)
